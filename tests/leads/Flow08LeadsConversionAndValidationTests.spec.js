...
test.describe.serial("Flow08 - Leads Conversion And Validation", () => {
  let page;
  let leadData;
  let leadSteps;
  let topMenuActions;
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    console.log(`Flow08 running with language: ${ENV.testLanguage}`);
  ...
  test("SearchAndConvertExistingLead", async () => {
    test.setTimeout(60000); // 60 second timeout for lead creation
    leadData = await leadSteps.searchAndConvertLead();
  });

  test("ValidateConvertedRecords", async () => {
    test.setTimeout(60000); // 60 second timeout for lead creation
    await leadSteps.validateLeadConversion(leadData);
  });
});
