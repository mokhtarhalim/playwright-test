...
test.describe.serial("Flow07 - Leads Deletion", () => {
  let page;
  let leadSteps;
  let topMenuActions;
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    console.log(`Flow07 running with language: ${ENV.testLanguage}`);
  ...
  test("DeleteLead", async () => {
    test.setTimeout(60000); // 60 second timeout for lead creation
    await leadSteps.searchAndDeleteLead();
  });

  test("ValidateLeadDeletion", async () => {
    test.setTimeout(60000); // 60 second timeout for lead creation
    await leadSteps.validateLeadDeletion();
  });
});
